...
const API = function (_client, masterPlayer) {
  ...
  app.post("/yoduhboat/api/search", async (req, res) => {
    ...
    // spotify
      ...
      // single spotify song (return 10 search results)
        ...
        results = await play.search(
          `${spotifyData.artists[0].name} ${spotifyData.name}`,
          {
            limit: 10,
          }
        );
    ...
    if (results.type === "playlist") {
      ...
      let thumbnail =
        results.thumbnail?.url ?? results.videos[0].thumbnails[0].url;
  ...
  app.get("/yoduhboat/api/playlists", async (req, res) => {
    try {
      ...
      let dbGuild = await Guild.findOne({ guildId: guildId })
        .populate("playlists", ["createdBy", "duration", "name", "songs", "id"])
        .exec();
      ...
      for (let i = 0; i < playlists.length; i++) {
        ...
        formattedResults.push({
          id: playlist.id,
          name: playlist.name,
          songsNum: playlist.songs.length,
          duration: duration,
          createdBy: user.global_name,
        });
      }
  ...
  app.get("/yoduhboat/api/playlist", async (req, res) => {
    ...
    let playlist = await Playlist.findById(playlistId)
      .populate({ path: "songs", options: { sort: { order: "asc" } } })
      .populate("createdBy", "global_name");
    if (!playlist) {
  ...
  app.post("/yoduhboat/api/playlist/removesong", async (req, res) => {
    ...
    const message = await generateFakeMessage(userId, guildId);
    try {
      await commands.playlist(
        ["remove", playlistName, index],
        message,
        null,
        true
      );
    ...
    try {
      if (Array.isArray(songs) && songs.length > 0) {
        for (const song of songs) {
          const dbSong = await Song.findById(
            new mongoose.Types.ObjectId(song.id)
          );
  ...
  app.get("/yoduhboat/api/history", async (req, res) => {
    const { guildId } = req.query;
    try {
      const dbHistory = await History.find({ guildId: guildId })
        .sort({ createdAt: "desc" })
        .populate("song", ["title", "artist", "link"])
        .populate("user", "global_name");
